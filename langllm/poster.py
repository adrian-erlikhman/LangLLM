...
def build():
    ...
    col1 = [
        ...
        Paragraph("<b>12 prompt schemas</b>, each fixing topic, stance (6 for / 6 against) and three sub-claims. A non-subject model (Llama 4) writes a native prompt per language — never a translation — and a third-family reviewer (Qwen) verifies every prompt against its schema. Content is held fixed; only style is free.", S["b"]),
    ...
    col4 = [
        ...
        Paragraph("6 · The models themselves cannot do this", S["h"]),
        img("F10_rq6_judge", fw),
        Paragraph(f"Each model asked which of the five wrote each text (reasoning off, options shuffled). GPT {j6.loc['gpt','accuracy']:.1%}, Grok {j6.loc['grok','accuracy']:.1%}, Gemini {j6.loc['gemini','accuracy']:.1%}, DeepSeek {j6.loc['deepseek','accuracy']:.1%}: chance, each defaulting to one answer (GPT names itself 82% of the time). Only Claude beats chance ({j6.loc['claude','accuracy']:.1%}, p &lt; 0.001) with a small self-recognition signal (own-text recall {j6.loc['claude','own_recall']:.0%} vs {j6.loc['claude','false_self_rate']:.0%} false claims), strongest in Hindi.", S["cap"]),
        Spacer(1, 6),
